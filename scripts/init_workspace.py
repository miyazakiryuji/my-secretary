...
使い方:
    ...
    python3 init_workspace.py --plugin-root <プラグインのパス>
        # プラグインの workspace-skills/ から、この執務室専用のスキル13点
        # （予定・締切/返事待ち・思い出し・申し送り・学習・人物・代筆・定型業務・
        #   自分仕様・繰り返し・見積もり・取り込み・決めたこと）を .claude/skills/ へ複製する
...
STANDARD_DIRS = [
    ...
    "月次レビュー",  # 月次のふりかえりと目標の照合（YYYY-MM.md）v1.3.0〜
]
...
| 月末・月初 | `/my-secretary:monthly` … 1か月分をまとめて、目標とのずれを見る |
...
| 「毎週月曜10時に定例」「毎月1日に請求書」 | 繰り返しとして登録。朝会で当日分を自動で載せる |
| 「請求書の手順を覚えて」→「請求書やろう」 | 定型業務の手順を覚え、次回は1手順ずつ一緒に進める（やるほど手順が磨かれる） |
| 「30分くらいで◯◯」 | 見積もりつきで記録。夕会で実績を聞き、見積もりの癖を覚える |
| 「この議事録取り込んで」「この名刺を人物メモに」 | 読み取って、人物・予定・タスクへ振り分け（承認してから） |
| 「朝会で体調も聞いて」「読書リストのフォルダ足して」 | 朝会・夕会の項目や管理する領域を自分仕様にする |
| 「これ全部さばいて」（複数行のメモを貼る） | 行ごとにタスク・予定・メモ・決めたことへ仕分け（承認してから。1行も落とさない） |
| 「おはよう、急ぎ」 | 質問なしのショート版朝会。秘書が★を決めて1メッセージで |
| 「◯◯にすると決めた」「なぜ◯◯にしたんだっけ」 | 判断を理由つきで残し、あとで原文と日付で思い出せる |
...
| 決めたこと.md | 判断と理由の記録（撤回しても消しません） |
| 繰り返し.md | 毎週・毎月くり返す予定とタスク（朝会で当日分を展開） |
| 朝会・夕会の項目.md | 朝会・夕会で必ず聞くこと（自分で増やせます） |
| 人物/・下書き/・議事録/ | 人物メモ・代筆した文章・打ち合わせの取り込み（必要になったとき秘書が作ります） |
| 業務手順/ | 定型業務のレシピ（「◯◯の手順を覚えて」で作られます） |
| 受信箱/ | 何でも書き捨てるメモ置き場 |
| タスク/ | 1日1ファイルのタスクリスト |
| アイデア/ | 1アイデア1ファイル |
| ノート/ | 調べたこと・ナレッジ |
...
（隠しフォルダ `.claude/skills/` には、この執務室**専用**の秘書スキル13点
〔予定・締切/返事待ち・思い出し・申し送り・学習・人物/打ち合わせ準備・代筆・定型業務・
自分仕様・繰り返し・見積もり・取り込み・決めたこと〕が入っています。触らなくて大丈夫ですが、
中身を開いて自分好みに育てるのも自由です）

仕事に合わせた気くばり（営業・事務・管理職・フリーランス・講師・学生・家庭）は
`/my-secretary:role` でいつでも決め直せます。
